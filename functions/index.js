...
const axios = require("axios");
const crypto = require("crypto");
// The 'defineString' import is removed for this test
// const { defineString } = require('firebase-functions/params');

admin.initializeApp();
const db = admin.firestore();

// --- TEMPORARY DIAGNOSTIC VALUES ---
// We are using fake placeholders to bypass Secret Manager
const PHONEPE_MERCHANT_ID = "FAKE_MERCHANT_ID";
const PHONEPE_SALT_KEY = "FAKE_SALT_KEY";
...
exports.phonePePay = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    // The PhonePe API will reject this, which is EXPECTED for this test.
    // We are looking to see if the function itself runs without a 401 error.
    if (response.data.success) {
      const redirectUrl = response.data.data.instrumentResponse.redirectInfo.url;
      await db.collection("orders").doc(orderId).update({ merchantTransactionId });
      return { redirectUrl };
    ...
    if (error.response && error.response.data) {
        const detailedMessage = error.response.data.message || JSON.stringify(error.response.data);
        // THIS is the expected success outcome for our test
        throw new functions.https.HttpsError("internal", `DIAGNOSTIC SUCCESS - PhonePe API failed as expected: ${detailedMessage}`);
    }
    throw new functions.https.HttpsError("internal", "Failed to initiate payment due to an unexpected server error.");
  }
});

// The callback function is not needed for this test but is kept for completeness
exports.phonePeCallback = functions.https.onRequest(async (req, res) => {
    res.status(200).send("Callback endpoint is active.");
});
